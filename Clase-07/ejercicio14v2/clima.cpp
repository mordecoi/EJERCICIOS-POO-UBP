...
Clima::Clima(QObject *parent) : QObject(parent) {
    ...
    connect(manager, &QNetworkAccessManager::finished, this, &Clima::respuestaRecibida);
    //connect(manager, SIGNAL(finished()), this, SLOT(respuestaRecibida));
}

void Clima::obtenerTemperatura(const QString &ciudad) {
...
void Clima::respuestaRecibida(QNetworkReply *reply) {
